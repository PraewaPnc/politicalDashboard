...
// ===============================
// Cache & Staging Helpers (Unchanged)
// ===============================
...
// ===============================
// NEW: File Staging Fallback
// ===============================
/**
 * Attempts to fetch the processed JSON file generated by the server-side pipeline.
 */
async function fetchFileStaging(filePath) {
  try {
    // Note: The path './data_cache/...' must be publicly accessible on your web server
    const res = await fetch(filePath);
    if (!res.ok) throw new Error("Staging file not found or inaccessible.");
    const obj = await res.json();
    // The server pipeline stores data as { timestamp: ..., data: [...] }
    return obj.data; 
  } catch (e) {
...
// ===============================
// Public: Fetch Vote Data (UPDATED for File Staging)
// ===============================
export async function fetchVoteData() {
  // 1️⃣ Try client-side cache first (TTL checked)
  ...
  // 2️⃣ Fetch live data
  console.log("🌐 Fetching vote data from GraphQL...");
  const { voteEvents = [] } = await fetchGraphQL(QUERY_VOTE_EVENTS);
  
  // 3️⃣ Fetch the cleaned party map
  console.log("Fetching cleaned party name map...");
  const cleanedPartyMap = await fetchCleanedPartyMap();

  // 4️⃣ Fallback to staging if fetch fails
  if (!voteEvents.length) {
    console.warn("⚠️ GraphQL failed. Checking for fallbacks...");
    
    // 🥇 Priority 1: Check server file staging
    ...
    if (fileStaging) {
      ...
      // OPTIONAL: Write server file data to browser cache for next time
      writeStorage(CACHE_KEY, fileStaging, true); 
      return fileStaging;
    }

    // 🥈 Priority 2: Fallback to browser's localStorage staging (last resort)
  ...
  // 5️⃣ Transform + Save (Only run if fresh data was successfully fetched)
  const records = transformVoteEvents(voteEvents, cleanedPartyMap);
  writeStorage(CACHE_KEY, records, true);  // Cache for 24h
  writeStorage(STAGING_KEY, records);     // Permanent local staging
...
// ===============================
// Public: Force Refresh (UPDATED for File Staging)
// ===============================
export async function forceRefreshVoteData() {
  ...
  if (!voteEvents.length) {
    ...
    // Fallback to the reliable server file staging instead of old localStorage
    const fileStaging = await fetchFileStaging('./data_cache/vote_data_cache.json');
    return fileStaging;
  }
